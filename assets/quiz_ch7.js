/* ======================================================
   Projektmanagement – Kapitel 7 Quiz Engine
   Themen:
   - Schätzmethoden (Pflicht)
     Expertenschätzung · Drei-Punkt-Verfahren · Planning Poker
     Additive & lineare Hochrechnung (Rechnen!)
   Strategie:
   - Normalmodus: 12 zufällig
   - Klausurmodus (?mode=exam): Rechnen + Transfer priorisiert
...
(function () {
  ...
  const STATS_KEY = "pmquiz_stats_ch7";
  const RUN_KEY   = "pmquiz_run_ch7";
  ...
  function mustExist(el, id) {
    if (!el) {
      console.error(`Quiz CH7: Missing element #${id}. Check chapter7.html.`);
      return false;
    }
    return true;
  }
  ...
  // --- Questions: dynamic access
  function getQuestions() {
    const q = window.QUESTIONS_CH7;
  ...
  function showFatal(msg) {
    quizRoot.innerHTML = `
      <div class="question">
        ...
        <p class="muted">Check: <code>window.QUESTIONS_CH7?.length</code></p>
      </div>
    `;
  }
  ...
  function createRun() {
    ...
    if (mode === "exam" && hasTypes(questions)) {
      // Kapitel 7 ist "Rechnen-Pflicht" → calculation + transfer hoch
      const QUOTA = { basic: 2, compare: 2, transfer: 4, calculation: 4 };
  ...
  function render() {
    ...
    if (missing > 0) {
      console.warn("Quiz CH7: Run contains unknown question IDs. Recreating run.");
      clearRun();
      resetRunUI();
      run = createRun();
      if (!run) return;
