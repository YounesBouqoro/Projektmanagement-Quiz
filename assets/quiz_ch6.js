/* ======================================================
   Projektmanagement – Kapitel 6 Quiz Engine
   Themen:
   - Projektabschluss
   - Abschlussbericht
   - Abnahmeprotokoll
   ...
   - Klausurmodus (?mode=exam): Transfer priorisiert
...
(function () {
  ...
  const STATS_KEY = "pmquiz_stats_ch6";
  const RUN_KEY   = "pmquiz_run_ch6";
  ...
  function mustExist(el, id) {
    if (!el) {
      console.error(`Quiz CH6: Missing element #${id}. Check chapter6.html.`);
      return false;
    }
    return true;
  }
  ...
  function showFatal(msg) {
    quizRoot.innerHTML = `
      <div class="question">
        ...
        <p class="muted">Check: <code>window.QUESTIONS_CH6?.length</code></p>
      </div>
    `;
  }
  ...
  function createRun() {
    ...
    if (mode === "exam" && hasTypes(questions)) {
      // Kapitel 6: Transfer heavy (Abnahme/Abschluss sauber begründen)
  ...
  function render() {
    ...
    if (missing > 0) {
      console.warn("Quiz CH6: Run contains unknown question IDs. Recreating run.");
      clearRun();
      resetRunUI();
      run = createRun();
      if (!run) return;
