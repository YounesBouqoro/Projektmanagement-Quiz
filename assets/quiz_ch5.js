/* ======================================================
   Projektmanagement – Kapitel 5 Quiz Engine
   Themen:
   - Projektplanung, -kontrolle & -steuerung
   - Regelkreis
   - Soll-Ist-Vergleichle
   - Trendanalysen (MTA, KTA, ATA)
   - Steuerungsmaßnahmen + Nebenwirkungen
   Strategie:
   - Normalmodus: 12 zufällig
   - Klausurmodus (?mode=exam): Transfer + Kontrolle/Trend priorisiert
...
(function () {
  ...
  const STATS_KEY = "pmquiz_stats_ch5";
  const RUN_KEY   = "pmquiz_run_ch5";
  ...
  function mustExist(el, id) {
    if (!el) {
      console.error(`Quiz CH5: Missing element #${id}. Check chapter5.html.`);
      return false;
    }
    return true;
  }
  ...
  function showFatal(msg) {
    quizRoot.innerHTML = `
      <div class="question">
        ...
        <p class="muted">Check: <code>window.QUESTIONS_CH5?.length</code></p>
      </div>
    `;
  }
  ...
  function createRun() {
    ...
    if (mode === "exam" && hasTypes(questions)) {
      // Kapitel 5: Rechnen/Trend/Steuerung ist klausurentscheidend → Transfer+Calculation hoch
      // (Wenn du später mehr Rechenfragen ergänzt, kann calculation > 0 werden)
      const QUOTA = { basic: 2, compare: 3, transfer: 5, calculation: 2 };
  ...
  function render() {
    ...
    if (missing > 0) {
      console.warn("Quiz CH5: Run contains unknown question IDs. Recreating run.");
      clearRun();
      resetRunUI();
      run = createRun();
      if (!run) return;
